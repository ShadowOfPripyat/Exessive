...
from PyQt5.QtWidgets import (QMainWindow, QTableView, QApplication, QToolBar, QLineEdit, QComboBox, QDialog,
                             QAction, QMenu, QFileDialog, QAbstractItemView, QMessageBox, QTabWidget, QVBoxLayout, QWidget)
...
class PandasModel(QAbstractTableModel):
    ...
    def setData(self, index, value, role):
        row = int(index.row())  # Ensure row is an integer
        col = int(index.column())  # Ensure col is an integer
        self._df.iloc[row, col] = value  # Access DataFrame using .iloc
...
class Viewer(QMainWindow):
    ...
    def createToolBar(self):
        openAction = QAction(QIcon.fromTheme("document-open"), "Open", self, triggered=self.loadCSV,
                             shortcut=QKeySequence.Open)
        saveAction = QAction(QIcon.fromTheme("document-save"), "Save", self, triggered=self.writeCSV_update,
                             shortcut=QKeySequence.Save)
        saveAsAction = QAction(QIcon.fromTheme("document-save-as"), "Save as ...", self, triggered=self.writeCSV,
    ...
    def openCSV(self, path):
        f = open(path, 'r+b')
        with f:
            if path.endswith('.xlsx'):  # Check if it's an Excel file
                df = pd.read_excel(f)
    ...
    def openFile(self, path=None):
        ...
        if self.model.setChanged == True:
            ...
            if reply == QMessageBox.Yes:
                ...
                return
        path, _ = QFileDialog.getOpenFileName(self, "Open File", QDir.homePath() + "/Documents/CSV/",
                                              "Excel Files (*.xlsx);;CSV Files (*.csv);;OLD Excel Files (*.xls)")
        if path:
            return path
